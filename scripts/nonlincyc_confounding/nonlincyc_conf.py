...
# non-linear cyclic time series functions
def tf5(n_T):
    P=39
...
for i in range(n_trials):
    ...
    data_object.non_linear_cyclic_modes.add_non_linear_modes(n_nonlinear_modes)
    data_object.non_linear_cyclic_modes.implement_timeseries_functions(fns=nonlinear_ts_fns, 
                                                                vrs=[1])
    ...
    if i <3:
        extprint('- saving model_projections')
