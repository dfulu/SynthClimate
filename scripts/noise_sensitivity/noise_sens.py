...
def instantiate_model(test, noise_var=1):    
    if modname=='sfa':
        if noise_var==0:
            return test.instantiate_model(n_components=8, pca_components=8)
        else:
...
directory_base  = '/exports/csce/datastore/geos/users/s1205782/evaluation/{}'.format(modname) + \
                  '/noise_sensitivity/trial_{:03}_noise_var_{:.1f}'

# Key test numbers
n_modes=12 # number of model modes
n_T = 14400 # length of time series
noise_vrs = [0,0.5,1,2,4,8,16,32]
repeats=4 
# Derived
noise_vrs = np.repeat(noise_vrs, repeats=repeats)
seeds = 82737 + np.arange(len(noise_vrs))
...
for i in range(len(noise_vrs)):
    directory = directory_base.format(i, noise_vrs[i])
    print(directory)
    ...
    data_object.noise_modes.implement_timeseries_functions(vr=noise_vrs[i])
    ...
    instantiate_model(test, noise_var=noise_vrs[i])
