...
directory_base  = '/exports/csce/datastore/geos/users/s1205782/evaluation/{}'.format(modname) + \
                  '/nonlinvar_conf/trial_{:03}_leading_var_{:02}'
...
repeats = 6
# Derived
mode_vrs = np.repeat(leading_vars, repeats=repeats, axis=0)
...
# RUN
#################################
for i, vr, seed in zip(trial_ns, mode_vrs, seeds):
    
    directory = directory_base.format(i, vr)
